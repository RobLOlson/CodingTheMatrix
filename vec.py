...
def list2Vec(L: list):
    return Vec(set(range(len(L))), {pre: L[pre] for pre in range(len(L))})
...
def equal(u, v):
    ...
    """
    assert u.D == v.D
    return all(u.f.get(pre, 0) == u.f.get(pre, 0) for pre in u.D)
...
def add(u, v):
    ...
    """
    assert u.D == v.D
    return Vec(u.D, {pre: u.f.get(pre, 0) + v.f.get(pre, 0) for pre in u.D})
...
def dot(u, v):
    ...
    """
    assert u.D == v.D
    return sum(int(u.f.get(pre, 0)) * int(v.f.get(pre, 0)) for pre in u.D)
...
def scalar_mul(v, alpha):
    ...
    True
    """
    return Vec(v.D, {key: value * alpha for key, value in v.f.items()})
...
def neg(v):
    ...
    True
    """
    return Vec(v.D, {pre: v.f.get(pre, 0) * -1 for pre in v.D})
...
class Vec:
    ...
    def from_list(cls, L: list):
        return cls(set(range(len(L))), {pre: L[pre] for pre in range(len(L))})
